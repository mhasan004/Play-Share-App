...
{             
    ...
    try{        
        // 2.1) Verify Refresh Token Token
        try{
        ...
        // 2.2) Verify Access Token -> Return with -1 prompting user to relogin 
        try{
            verified_access = verifyToken(recieved_access_token)
        }
        catch(err){
            return res.status(401).json({status: -2, message: "Access Denied! May need to refresh Access Token. Error:"+err}).end()
        }
    }
    catch(err){
    ...
    req.jwtPayload = verified_access                                                                                                         // req.jwtPayload = JWT payload
    next()
}
